...
async def broadcast_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "broadcast:confirm":
        ...
        if not payload:
            await query.edit_message_text(
                "No pending broadcast. Start again from Admin → Broadcast.",
                reply_markup=back_to_admin_keyboard(),
            )
            return
        ...
        await clear_admin_state(user_id)
        await query.edit_message_text(
            "📢 Broadcast queued. It will be sent in the background. You can continue using the bot.",
            reply_markup=back_to_admin_keyboard(),
        )
...
async def capture_message_for_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """When admin is in broadcast:wait_message, store the message and ask for confirmation."""
    user_id = update.effective_user.id if update.effective_user else 0
...
def _message_to_payload(update: Update) -> dict | None:
    """Extract type, content, file_id from update.message for broadcast."""
    msg = update.message
    if not msg:
        return None
...
async def _send_one_broadcast(bot, user_id: int, payload: dict) -> bool:
    """Return True if sent, False if skipped (blocked)."""
    try:
...
async def broadcast_worker(bot) -> None:
    """Background task: pop from Redis queue and send with rate limit. Run as asyncio.create_task."""
