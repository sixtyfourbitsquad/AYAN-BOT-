"""Start command: admin panel for admins, simple reply for users."""
...
async def update_last_seen(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Update user last_seen on every interaction (messages only)."""
...
def register_start(app) -> None:
    ...
    app.add_handler(
        MessageHandler(filters.ChatType.PRIVATE & ~filters.COMMAND, update_last_seen),
        group=0,
    )
